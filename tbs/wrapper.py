...
class Wrapper(SerializableMixin):
    cls_game = Game
    cls_event_factory = EventFactory
    ...
    def encode(self):
        return {
            ...
            'events': [self.cls_event_factory.encode(event) for event in self.events],
        }

    @classmethod
    def decode(cls, **kwargs):
        decoded_settings = kwargs['settings']
        decoded_events = [cls.cls_event_factory.decode(**e_kwargs) for e_kwargs in kwargs['events']]
